...
|---|---:|---:|---|
| **Windows 10/11 x64** | **Portable Python + FFmpeg** | ✅ Recommended | Primary target layout for the included launcher and installer. |
| **Windows 10/11 x64** | **NVIDIA NVENC** | ✅ Supported | Optional acceleration for H.264/H.265 rendering. Requires compatible NVIDIA GPU and drivers. |
| **Windows 10/11 x64** | **CPU Encoding** | ✅ Supported | Uses software FFmpeg encoders when NVENC is disabled or unavailable. |
| **Windows 10/11 x64** | **ProRes / FFV1** | ✅ Supported | High-quality and archival export paths. Larger files. |
| **AMD / Intel GPU** | **CPU fallback** | ⚠️ Supported through FFmpeg CPU paths | GPU acceleration is NVIDIA NVENC-focused. |
| **Linux / macOS** | **Manual Python run** | ⚠️ Not packaged target | The Python code is mostly portable, but the provided runtime scripts are Windows-focused. |

### Media support

| Input / Output | Supported |
|---|---|
| **Image input** | Common image formats supported by Gradio/Pillow and FFmpeg image input. |
| **Video input** | `.mp4`, `.mkv`, `.avi`, `.mov`, `.webm`, `.wmv`, `.flv`, `.mpeg`, `.mpg`, `.m4v`, `.ogv` |
| **LUT input** | `.cube` |
| **LUT output** | `.cube` 3D LUT files |
| **Image output** | `.png` from preview/save flows, `.png` or `.jpg` from Merge Mode |
| **Video output** | `.mp4`, `.mkv`, or `.mov`, depending on selected codec |
...
| Component | Role |
|---|---|
| **Python** | Runs the application and color-processing code. |
| **Gradio** | Provides the local browser-based interface. |
| **NumPy** | Handles LUT cube math and image array processing. |
| **Pillow** | Loads, converts, and saves preview images. |
| **FFmpeg** | Applies `.cube` LUT files to images and videos in Merge Mode. |
| **FFprobe** | Reads video duration for automatic and manual frame extraction. |
| **NVIDIA NVENC** | Optional hardware encoder for supported H.264/H.265 video exports. |
...
|---|---|
| **App does not open** | Confirm the portable Python runtime exists inside `bin/` and the launcher can find `src/app.py`. |
| **Video preview does not extract frames** | Confirm FFmpeg and FFprobe are available in `bin/ffmpeg/` or on the system PATH. |
| **Merge render fails** | Confirm both the media file and `.cube` LUT file exist, and try a CPU codec if NVENC fails. |
| **NVENC render fails** | Confirm the GPU supports NVENC, NVIDIA drivers are installed, and the selected codec is H.264 or H.265. |
| **LUT looks too strong** | Try a different preset, lower manual contrast/saturation, or switch logic engines. |
| **Extreme LUT is slow** | Use Ultra 65 Grid for a faster quality/performance balance. |

---

## 🧹 Updating

Before replacing the app folder, back up any generated work you want to keep.

Recommended folders to preserve:
...
If you keep custom icons, preview images, or local edits, also back up the relevant files inside `src/assets/` and `src/`.

---

## 🧽 Uninstall

LUT Colorator is portable. To uninstall it, close the app and delete the application folder.

This does not remove system-level GPU drivers, external FFmpeg installations, or other software installed outside the portable folder.
...
*   **NumPy:** Array computing library by the [NumPy project](https://numpy.org/).
*   **Pillow:** Python imaging library by the [Pillow project](https://python-pillow.org/).
*   **NVIDIA NVENC:** Hardware video encoding technology by [NVIDIA](https://developer.nvidia.com/).
...
LUT Colorator is released under the **Apache License 2.0**.
...
LUT Colorator is a local media-processing tool. Preview speed, render speed, and output quality depend on source media, LUT grid size, FFmpeg availability, selected codec, CPU performance, GPU support, and driver configuration.

Only process media you have the right to use. Large video files and high-quality render settings can consume significant disk space, CPU time, and GPU resources.
